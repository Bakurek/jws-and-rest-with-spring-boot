package com.pjankows.rest.webservices.restfulwebservices.exception;
...
import com.pjankows.rest.webservices.restfulwebservices.user.UserNotFoundException;
...
public class CustomizedReponseEntityExceptionHandler extends ResponseEntityExceptionHandler {
	...
	public final ResponseEntity<Object> handleAllExceptions(PostNotFoundException ex, WebRequest request) {
		ExceptionResponse exceptionResponse = new ExceptionResponse(new Date(), ex.getMessage(), request.getDescription(false));
		return new ResponseEntity(exceptionResponse, HttpStatus.NOT_FOUND);
	}
